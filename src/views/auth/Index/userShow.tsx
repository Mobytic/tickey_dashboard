...
const UserShow = ({ user }: UserShowProps) => {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h6 className="mb-4 text-gray-800 dark:text-gray-100 border-b pb-2">Informations de l'utilisateur</h6>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div><span className="font-semibold">Nom :</span> {user.lastname} {user.firstname}</div>
            ...
            {user.websites && user.websites.length > 0 && (
                <div>
                    <h6 className="mb-4 text-gray-800 dark:text-gray-100 border-b pb-2 mt-6">
                        Sites Web ({user.websites.length})
                    </h6>
